...
import {
  ...
  Sparkles, Upload, Trash2, Edit, Send, Eye, Lock, CheckCircle, Clock, Users, UserPlus,
} from "lucide-react";
...
export default function TestRepository() {
  ...
  const handleBulkAssignReviewer = async () => {
    if (!bulkReviewerId || selectedIds.length === 0) return;
    const eligibleCases = testCases.filter(
      (tc) => selectedIds.includes(tc.id) && tc.status === "submitted_for_review"
    );
    for (const tc of eligibleCases) {
    ...
    setSelectedIds([]);
    setBulkReviewerId("");
    toast({ title: "Reviewers assigned", description: `Assigned reviewer to ${eligibleCases.length} test case(s).` });
  };
  ...
  const stats = useMemo(() => {
    ...
    const inReview = testCases.filter((tc) => ["submitted_for_review", "in_review"].includes(tc.status)).length;
    const approved = testCases.filter((tc) => ["approved", "ready_for_execution"].includes(tc.status)).length;
    const pendingMyReview = testCases.filter((tc) => tc.assigned_reviewer === user?.id && ["submitted_for_review", "in_review"].includes(tc.status)).length;
    const awaitingApproval = testCases.filter((tc) => tc.status === "reviewed").length;
    return { total, draft, inReview, approved, pendingMyReview, awaitingApproval };
  }, [testCases, user?.id]);

  // Lifecycle funnel
  const funnelProgress = useMemo(() => {
    if (stats.total === 0) return 0;
    const readyCount = testCases.filter((tc) => ["ready_for_execution", "executed"].includes(tc.status)).length;
    return Math.round((readyCount / stats.total) * 100);
    ...
    <AppLayout>
      <div className="space-y-6">
            ...
            <>
              <Card className="border-border">
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-2xl font-bold text-foreground">{stats.pendingMyReview}</p>
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm text-muted-foreground whitespace-nowrap">Ready for Execution</span>
          <Progress value={funnelProgress} className="h-2 flex-1" />
          <span className="text-sm font-medium text-foreground">{funnelProgress}%</span>
        </div>
            ...
            <CardContent className="py-3 flex items-center gap-4 flex-wrap">
              ...
              {selectedSubmittedCount > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" disabled={!bulkReviewerId || assignReviewer.isPending} onClick={handleBulkAssignReviewer}>
                    <UserPlus className="h-3.5 w-3.5 mr-1" />
                    Assign ({selectedSubmittedCount})
                  </Button>
                </div>
              )}
